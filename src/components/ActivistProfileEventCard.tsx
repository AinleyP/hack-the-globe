  ...
  return (
    <div className="profile-event-card">
      ...
      <div className="profile-event-card-text">
        ...
        <p>
          {props.event.description ? `${props.event.description.substring(0, 100)}... ${props.event.location}` : ''}
        </p>
        <Tags
          tags={
            props.event.responsiblities
              ? props.event.responsiblities.map((responsibility: Responsibility) => responsibility.name)
              : []
          }
        />
      </div>
